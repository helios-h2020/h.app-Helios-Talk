...
public class TagEditText extends EditText {
    ...
    private void init() {
        ...
        setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus && !getText().toString().endsWith(",") && !getText().toString().equals("")) {
    ...
    public void format() {
        ...
        for (int i = 0; i < strings.length; i++) {
            ...
            if (fullString.charAt(fullString.length() - 1) !=
                    separator.charAt(0) && i == strings.length - 1) {
                break;
            }

            BitmapDrawable bd = (BitmapDrawable) convertViewToDrawable(
                    createTokenView(string));
            bd.setBounds(0, 0, bd.getIntrinsicWidth(), bd.getIntrinsicHeight());

            int startIdx = sb.length() - (string.length());
            int endIdx = sb.length();

            sb.setSpan(new ImageSpan(bd), startIdx, endIdx,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

            MyClickableSpan myClickableSpan =
                    new MyClickableSpan(startIdx, endIdx);
            sb.setSpan(myClickableSpan, Math.max(endIdx - 2, startIdx), endIdx,
    ...
    public View createTokenView(String text) {
        ...
        l.setBackgroundResource(R.drawable.tag_bubble);
        ...
        l.addView(im);
        im.setImageResource(R.drawable.ic_close);
    ...
    public ArrayList<String> getTags() {
        if (getText().toString().isEmpty()) return new ArrayList();
        String[] array = getText().toString().split(separator);
